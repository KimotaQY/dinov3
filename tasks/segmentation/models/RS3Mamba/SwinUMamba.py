...
class SS2D(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dt_projs = (
            self.dt_init(self.dt_rank, self.d_inner, dt_scale, dt_init, dt_min,
        ...
        self.dt_projs_weight = nn.Parameter(
            torch.stack([t.weight for t in self.dt_projs],
                        dim=0))  # (K=4, inner, rank)
        self.dt_projs_bias = nn.Parameter(
            torch.stack([t.bias for t in self.dt_projs],
                        dim=0))  # (K=4, inner)
        del self.dt_projs
    ...
    def forward_core(self, x: torch.Tensor):
        ...
        wh_y = torch.transpose(out_y[:, 1].view(B, -1, W, H), dim0=2,
...
class VSSMEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        if isinstance(dims, int):
            dims = [
                int(dims * 2**i_layer) for i_layer in range(self.num_layers)
            ]
