    ...
    def __init__(self,
        ...
        super(ConvBNReLU, self).__init__(
            nn.Conv2d(in_channels,
                      ...
                      2), norm_layer(out_channels), nn.ReLU6())
...
class GlobalLocalAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn_x = nn.AvgPool2d(kernel_size=(window_size, 1),
                                   stride=1,
                                   padding=(window_size // 2 - 1, 0))
        self.attn_y = nn.AvgPool2d(kernel_size=(1, window_size),
                                   stride=1,
...
class Block(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn = GlobalLocalAttention(dim,
                                         num_heads=num_heads,
                                         qkv_bias=qkv_bias,
...
class WF(nn.Module):
    ...
    def __init__(self, in_channels=128, decode_channels=128, eps=1e-8):
        ...
        self.weights = nn.Parameter(torch.ones(2, dtype=torch.float32),
                                    requires_grad=True)
        self.eps = eps
        self.post_conv = ConvBNReLU(decode_channels,
                                    decode_channels,
                                    kernel_size=3)
...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.pre_conv = ConvBN(encoder_channels[-1],
                               decode_channels,
                               kernel_size=1)
...
class ChannelAtt(nn.Module):
    ...
    def forward(self, x):
        ...
        self.pool = SoftPool2d(kernel_size=(x.size(2), x.size(3)),
                               stride=(x.size(2), x.size(3)))
        ...
        Att = torch.sigmoid(channel_att_sum).unsqueeze(2).unsqueeze(
            3).expand_as(x)
        return Att
...
class RS3Mamba(nn.Module):
    ...
    def forward(self, x):
        h, w = x.size()[-2:]
        ssmx = self.stem(x)
        vss_outs = self.vssm_encoder(
            ssmx)  # 48*128*128, 96*64*64, 192*32*32, 384*16*16, 768*8*8
...
def load_pretrained_ckpt(model, ckpt_path="./pretrain/vmamba_tiny_e292.pth"):
    ...
    for k, v in ckpt['model'].items():
        ...
        if kr in model_dict.keys():
            assert v.shape == model_dict[
                kr].shape, f"Shape mismatch: {v.shape} vs {model_dict[kr].shape}"
            model_dict[kr] = v
        else:
