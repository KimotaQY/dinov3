...
from tasks.segmentation.models.utils.batch_norm import get_norm
from tasks.segmentation.models.utils.position_encoding import PositionEmbeddingSine
from tasks.segmentation.models.utils.transformer import _get_clones, _get_activation_fn
...
class MSDeformAttnTransformerEncoderOnly(nn.Module):
    ...
    def __init__(
        ...
        self.level_encoding = nn.Parameter(
...
class MSDeformAttnPixelDecoder(nn.Module):
    ...
    def __init__(
        self,
        input_shape: Dict[
            str, Tuple[int]],  # ShapeSpec: [channels, height, width, stride]
        *,
        ...
        transformer_input_shape = sorted(transformer_input_shape.items(),
                                         key=lambda x: x[1][-1])
        ...
        self.transformer_feature_strides = [
            v[-1] for k, v in transformer_input_shape
        ...
        for idx, in_channels in enumerate(
                self.feature_channels[:1]):  # TODO self.num_fpn_levels]):
    ...
    def forward_features(self, features):
        ...
        for idx, f in enumerate(
                self.transformer_in_features[::-1][:-1]):  # TODO remove [:-1]
            x = features[f].float(
        ...
        for idx, f in enumerate(
                self.in_features[0]
        ):  # TODO re put [:self.num_fpn_levels][::-1]):
            x = features[f].float()
