...
from .vit_seg_modeling_resnet_skip import FuseResNetV2
...
class Attention(nn.Module):
    ...
    def __init__(self, config, vis, mode=None):
        ...
        if self.mode == 'mba':
            self.w11 = torch.nn.Parameter(torch.FloatTensor(1),
                                          requires_grad=True)
            self.w12 = torch.nn.Parameter(torch.FloatTensor(1),
                                          requires_grad=True)
            self.w21 = torch.nn.Parameter(torch.FloatTensor(1),
                                          requires_grad=True)
            self.w22 = torch.nn.Parameter(torch.FloatTensor(1),
                                          requires_grad=True)
    ...
    def forward(self, hidden_statesx, hidden_statesy):
        ...
        attention_scores = torch.matmul(queryd_layer,
                                        keyd_layer.transpose(-1, -2))
        attention_scores = attention_scores / math.sqrt(
            self.attention_head_size)
        attention_probs = self.softmax(attention_scores)
        weights = attention_probs if self.vis else None
...
class Block(nn.Module):
    ...
    def load_from(self, weights, n_block):
        ...
        with torch.no_grad():
            ...
            self.attention_normd.weight.copy_(
                np2th(weights[pjoin(ROOT, ATTENTION_NORM, "scale")]))
            self.attention_normd.bias.copy_(
                np2th(weights[pjoin(ROOT, ATTENTION_NORM, "bias")]))
            self.ffn_normd.weight.copy_(
                np2th(weights[pjoin(ROOT, MLP_NORM, "scale")]))
            self.ffn_normd.bias.copy_(
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids, dsm_ids):
        embeddingsx, embeddingsy, features = self.embeddings(
            input_ids, dsm_ids)
        encodedx, encodedy, attn_weights = self.encoder(
            embeddingsx, embeddingsy)  # (B, n_patch, hidden)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, y):
        x, y, attn_weights, features = self.transformer(
            x, y)  # (B, n_patch, hidden)
        x = x + y
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            self.transformer.embeddings.patch_embeddingsd.weight.copy_(
                np2th(weights["embedding/kernel"], conv=True))
            ...
            self.transformer.encoder.encoder_normd.weight.copy_(
                np2th(weights["Transformer/encoder_norm/scale"]))
            ...
            if self.transformer.embeddings.hybrid:
                ...
                self.transformer.embeddings.hybrid_model.root.gn.bias.copy_(
                    gn_bias)
                self.transformer.embeddings.hybrid_model.rootd.gn.weight.copy_(
                    gn_weight)
