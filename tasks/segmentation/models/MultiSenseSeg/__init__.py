from .multimodal_fuse.Build_multimodal_fuse_head import *
from .pipeline.Build_pipeline import *
from .neck.Build_neck import *
from .decode_gate.Build_decode_gate import *
...
class Build_MultiSenseSeg(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_faster=False):
        super().__init__()
        n_branch = len(in_chans) if isinstance(
            in_chans, tuple) else 1 if n_branch is None else n_branch
        ...
        self.build_neck = Build_neck(in_chans=embed_dim,
        ...
        if self.aux:
            self.aux_out = nn.Sequential(
                nn.Conv2d(embed_dim * 2**(len(depths) - 2),
                          decoder_chans // 2,
                          kernel_size=3,
                          padding=1,
                          bias=False),
                creat_norm_layer(fpn_norm_layer, decoder_chans // 2),
                fpn_act_layer(inplace=True)
                if fpn_act_layer != nn.GELU else fpn_act_layer(),
    ...
    def forward(self, x, y):
        ...
        if aux:
            ...
            aux_x = F.interpolate(aux_x,
                                  scale_factor=2**(len(self.depths)),
                                  mode='bilinear',
                                  align_corners=True)
            return x, aux_x
        else:
