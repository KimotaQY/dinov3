...
from .decode_gate_layers import *
    ...
    def __init__(self,
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_chans,
                      ...
                      kernel_size=3,
                      padding=1,
                      bias=False), creat_norm_layer(norm_layer, head_chans))

        self.spat_attn = Spatial_attention(
            encoder_chans=en_chans,
            decoder_chans=head_chans,
            attn_chans=None,
            act_layer=act_layer) if not dw_spac_attn else Dw_spatial_attention(
                head_chans)

        self.dwconv = nn.Sequential(
            nn.Conv2d(head_chans,
                      head_chans,
                      kernel_size=3,
                      padding=1,
                      groups=head_chans),
        ...
        if chan_attn_type == 'CBAM':
            self.chan_attn = CBAM_channel_attention(in_chans=head_chans,
                                                    ratio=chan_ratio)
        elif chan_attn_type == 'SE':
            self.chan_attn = SE_channel_attention(in_chans=head_chans,
                                                  ratio=chan_ratio)
        else:
            raise NotImplementedError(
                f"Build channel attention does not support {chan_attn_type}")
