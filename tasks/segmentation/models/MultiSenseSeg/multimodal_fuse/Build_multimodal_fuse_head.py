...
class Build_multimodal_fuse_head(nn.Module):
    ...
    def __init__(self,
                 ...
                 embed_dim=None):
        super().__init__()
        in_chans = in_chans if isinstance(in_chans, tuple) else tuple(
            [in_chans for _ in range(n_branch)])
        ...
        self.MSEs = nn.ModuleList([
            MSE(in_chans=in_chans[i],
                ...
                ratio=chan_ratio) for i in range(n_branch)
        ])
        ...
        if use_pos:
            ...
            self.pos = [
                nn.Parameter(
                    torch.tensor([np.cos(ang_table[i] * np.pi / 180)],
                                 dtype=torch.float32)) for i in range(n_branch)
            ]

        smooth_chans = int(out_chans * n_branch)
        self.smooth = nn.Sequential(
            nn.Conv2d(smooth_chans,
                      out_chans,
                      kernel_size=3,
                      padding=1,
                      bias=False), nn.BatchNorm2d(out_chans), nn.ReLU())
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.MSEs):
            x_branch = layer(
                x[i],
                self.pos[i].to(device=self.device) if self.use_pos else None)
