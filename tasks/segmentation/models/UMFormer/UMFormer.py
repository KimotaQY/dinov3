...
class MSAM(nn.Module):
    ...
    def __init__(self, dim_out):
        ...
        self.branch4 = nn.Sequential(nn.Conv2d(512, dim_out, kernel_size=1),
                                     nn.BatchNorm2d(dim_out), nn.ReLU6())
        self.merge = nn.Sequential(
            nn.Conv2d(4 * dim_out, dim_out, kernel_size=1),
            nn.BatchNorm2d(dim_out), nn.ReLU6())
        self.resblock = nn.Sequential(
            IndentityBlock(in_channel=dim_out,
                           kernel_size=3,
                           filters=[dim_out, dim_out, dim_out]),
            IndentityBlock(in_channel=dim_out,
                           kernel_size=3,
                           filters=[dim_out, dim_out, dim_out]))
    ...
    def __init__(self, dim_in, dim_out, rate=1):
        ...
        self.branch1_1 = nn.Sequential(
            nn.Conv2d(dim_in,
                      ...
                      padding=0,
                      dilation=rate,
                      bias=True), nn.BatchNorm2d(dim_out), nn.SiLU())
        self.branch1_2 = nn.Sequential(
            SeparableConvBNReLU(dim_in, dim_out, kernel_size=3))
        self.branch2 = IndentityBlock(in_channel=dim_in,
                                      kernel_size=3,
                                      filters=[dim_in, dim_out, dim_out],
                                      rate=1)
        self.branch3 = IndentityBlock(in_channel=dim_in,
                                      kernel_size=3,
                                      filters=[dim_in, dim_out, dim_out],
                                      rate=2)
        self.branch4 = IndentityBlock(in_channel=dim_in,
                                      kernel_size=3,
                                      filters=[dim_in, dim_out, dim_out],
                                      rate=3)
...
class FeatureEmbedding(nn.Module):
    ...
    def __init__(self, in_channel, out_channel):
        ...
        self.conv1 = SeparableConvBNReLU(in_channel,
                                         out_channel,
                                         kernel_size=3)
        self.conv2 = SeparableConvBNReLU(out_channel,
                                         out_channel,
                                         kernel_size=3)
...
class MambaBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.self_attention = Mamba_Block(depth=1,
...
class GL_VSS(nn.Module):
    ...
    def forward(self, x):
        ...
        # 确保globe和local是连续的再传递给featureembedding
        globe = globe.contiguous()
...
class Decoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 dims=[64, 128, 256, 512],
                 dims_decoder=[128, 64, 64, 64],
                 conformity_dim=[0, 384, 192, 128],
                 d_state=16,
                 drop_rate=0.,
                 attn_drop_rate=0.,
                 drop_path_rate=0.1,
        ...
        dpr_decoder = [
            x.item()
            for x in torch.linspace(0, drop_path_rate, sum(depths_decoder))
        ][::-1]
        for i_layer in range(self.num_layers):
            layer = GL_VSS(
                ...
                if d_state is None else d_state,  # 20240109
                drop=drop_rate,
    ...
    def forward(self, x, skip_list):
        ...
        x = self.finalconv(
            F.interpolate(x, (h * 32, w * 32), None, 'bilinear', True))
        return x


class UMFormer(nn.Module):

    def __init__(self, num_classes, local_model_dir=None):
        super(UMFormer, self).__init__()

        # 如果提供了本地模型目录，则从本地加载
        if local_model_dir and os.path.exists(local_model_dir):
            # 从本地目录加载 Hugging Face 模型
            self.backbone = timm.create_model("resnet18.fb_swsl_ig1b_ft_in1k",
                                              features_only=True,
                                              output_stride=32,
                                              out_indices=(1, 2, 3, 4),
                                              pretrained=False)
            ...
            try:
                ...
                self.backbone = timm.create_model(
                    ...
                    output_stride=32,
                    out_indices=(1, 2, 3, 4),
                    pretrained_cfg_overlay=dict(file=local_model_dir +
                                                "/pytorch_model.bin"))
            except Exception as e:
                print(
                    f"Failed to load from {local_model_dir}/pytorch_model.bin: {e}"
                )
                # 备用方案：使用默认预训练权重（需要网络连接）
                self.backbone = timm.create_model(
                    "resnet18.fb_swsl_ig1b_ft_in1k",
                    features_only=True,
                    output_stride=32,
