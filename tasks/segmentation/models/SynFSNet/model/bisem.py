...
class BidimensionalSpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = self.channel_attention(
            F.adaptive_avg_pool2d(self.conv_3x3(x), [height, 1]))
        k = self.channel_attention(F.adaptive_avg_pool2d(x, [1, width]))

        v1 = F.relu(
            self.bn_conv_1x1_2(
                self.conv_1x1_2(
                    F.adaptive_avg_pool2d(x, [height // 2, width // 2]))))
        v1 = F.interpolate(v1,
                           size=x.size()[2:],
                           mode='bilinear',
                           align_corners=True)
        ...
        v2 = F.relu(
            self.bn_conv_1x1_2(
                self.conv_1x1_2(
                    F.adaptive_avg_pool2d(x, [height // 6, width // 6]))))
        v2 = F.interpolate(v2,
                           size=x.size()[2:],
                           mode='bilinear',
                           align_corners=True)
        ...
        v3 = F.relu(
            self.bn_conv_1x1_2(
                self.conv_1x1_2(
                    F.adaptive_avg_pool2d(x, [height // 8, width // 8]))))
        v3 = F.interpolate(v3,
                           size=x.size()[2:],
                           mode='bilinear',
                           align_corners=True)
        ...
        attention_scores = attention_scores / torch.sqrt(
            torch.tensor(self.out_ch // 2, dtype=torch.float32))
        attention_weights = torch.nn.functional.softmax(attention_scores,
                                                        dim=-1)
...
class BiSEM(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, height):
        ...
        self.BiSA = BidimensionalSpatialAttention(in_channels, out_channels,
                                                  height)
