...
def window_reverse(windows, window_size, H, W):
    ...
    x = windows.view(B, H // window_size[0], W // window_size[1],
                     window_size[0], window_size[1], -1).contiguous()
...
class AFSIAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.HSLFPN = nn.Sequential(nn.Linear(dim, dim // 4), nn.ReLU(),
    ...
    def forward(self, x):
        ...
        if self.use_relative_pe and N == 16:
            relative_position_bias = self.relative_position_bias_table[
                self.relative_position_index.view(-1)].view(16, 16, -1)
            relative_position_bias = relative_position_bias.permute(
                2, 0, 1).contiguous()  # nH, 16, 16
        ...
        if self.frequency_weight > 0:
            try:
                ...
                freq_attn = freq_attn.repeat(1, self.num_heads, 1,
                                             1)  # [B_, num_heads, N, N]

                adaptive_weight = torch.sigmoid(self.frequency_weight)

                combined_attn = (
                    1 - adaptive_weight) * attn + adaptive_weight * freq_attn
                attn = combined_attn
            except Exception as e:
                pass
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, act_layer=nn.GELU, drop=0.):
        ...
        self.dwconv2 = nn.Conv2d(in_features,
                                 in_features // 4,
                                 kernel_size=3,
                                 stride=1,
                                 padding=1)
        self.dwconv3 = nn.Conv2d(in_features,
                                 in_features // 4,
                                 kernel_size=7,
                                 stride=1,
                                 padding=3)
        ...
        self.fc1 = nn.Conv2d(in_features,
                             ...
                             stride=1,
                             padding=0)
        self.fc2 = nn.Conv2d(in_features * 4,
                             in_features,
                             kernel_size=1,
                             stride=1,
                             padding=0)
...
class AFSIFormer(nn.Module):
    ...
    def __init__(self, dim, heads):
        ...
        self.h_conv = nn.Conv2d(dim // self.reduced_channel,
                                dim // self.reduced_channel, 2, 2, 0)
        self.w_conv = nn.Conv2d(dim // self.reduced_channel,
                                dim // self.reduced_channel, 2, 2, 0)
        ...
        self.conv_out = nn.Conv2d(dim // self.reduced_channel,
                                  dim,
                                  3,
                                  1,
                                  1,
        ...
        self.mlp = Mlp(in_features=dim // self.reduced_channel,
                       act_layer=nn.GELU,
                       drop=0.1)
    ...
    def forward(self, x):
        ...
        if pad_r > 0:
            x_down = F.pad(x_down, (0, 0, pad_l, pad_r, pad_t, pad_b),
                           mode='reflect')

        border_index = torch.Tensor(self.borderindex(
            x_down.shape[2])).int().to(x_down.device)

        x_h = torch.index_select(x_down, 1,
                                 border_index).permute(0, 3, 1,
                                                       2).contiguous()
        ...
        x_w = torch.index_select(x_down, 2,
                                 border_index).permute(0, 3, 2,
                                                       1).contiguous()
        ...
        x_h, x_w = window_reverse(x_h, [1, w_], h_,
                                  w_).permute(0, 3, 1,
                                              2).contiguous(), window_reverse(
                                                  x_w, [1, w_], h_,
                                                  w_).permute(0, 3, 2,
                                                              1).contiguous()
        x_h, x_w = F.interpolate(x_h,
                                 scale_factor=2,
                                 mode='bilinear',
                                 align_corners=True), F.interpolate(
                                     x_w,
                                     scale_factor=2,
                                     mode='bilinear',
                                     align_corners=True)
        x_h, x_w = x_h.permute(0, 2, 3,
                               1).contiguous(), x_w.permute(0, 2, 3,
                                                            1).contiguous()

        x_down.index_add_(1, border_index, x_h)
        x_down.index_add_(2, border_index, x_w)

        lwam_local = window_partition(x_down, [4, 4]).view(
            -1, 16, x_down.shape[3]).contiguous()
        ...
        if pad_r > 0:
            x_down = x_down[:, pad_t:H + pad_t,
                            pad_l:W + pad_t, :].contiguous()
            lwam_local = lwam_local[:, pad_t:H + pad_t,
                                    pad_l:W + pad_t, :].contiguous()
        ...
        output = F.interpolate(lwam_local,
                               scale_factor=2,
                               mode='bilinear',
                               align_corners=True)
...
def convnext_small(pretrained=False, in_22k=False, **kwargs):
    ...
    if pretrained:
        url = model_urls['convnext_small_22k'] if in_22k else model_urls[
            'convnext_small_1k']
...
class upsample(nn.Module):
    ...
    def __init__(self, in_c, factor):
        ...
        self.up = nn.Sequential(
            nn.Conv2d(in_c,
                      self.factor1 * in_c, (1, 7),
                      padding=(0, 3),
                      groups=in_c),
            nn.Conv2d(self.factor1 * in_c,
                      self.factor2 * in_c, (7, 1),
                      padding=(3, 0),
                      groups=in_c), nn.PixelShuffle(factor),
...
class SynFSNet(nn.Module):
    ...
    def __init__(self, n_class=6, pretrained=True):
        ...
        self.seghead = nn.Conv2d(config[0],
                                 n_class,
                                 kernel_size=1,
                                 stride=1,
                                 padding=0)
        self.BiSEM = BiSEM(in_channels=config[0],
