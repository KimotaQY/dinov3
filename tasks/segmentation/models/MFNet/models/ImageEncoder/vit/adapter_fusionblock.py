...
class AdapterFusionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.MLPx_Adapter = Adapter(
            adapter_dim, skip_connect=False)  # MLP-adapter, no skip connection
        self.MLPy_Adapter = Adapter(
            adapter_dim, skip_connect=False)  # MLP-adapter, no skip connection
        ...
        self.mlp = MLPBlock(embedding_dim=dim,
                            ...
                            act=act_layer)
        self.wx_Adapter = torch.nn.Parameter(torch.FloatTensor(1),
                                             requires_grad=True)
        self.wy_Adapter = torch.nn.Parameter(torch.FloatTensor(1),
                                             requires_grad=True)
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor = None):
        ...
        if y is not None:
            ...
            x = x + self.mlp(
                xn) + self.wx_Adapter * adax + (1 - self.wx_Adapter) * aday
            y = y + self.mlp(
                yn) + self.wy_Adapter * aday + (1 - self.wy_Adapter) * adax
