    ...
    def __init__(self,
        ...
        super(ConvBNReLU, self).__init__(
            nn.Conv2d(in_channels,
                      ...
                      2,
                      groups=groups), norm_layer(out_channels), nn.ReLU6())
    ...
    def __init__(self,
        ...
        super(SeparableConvBNReLU, self).__init__(
            nn.Conv2d(in_channels,
                      ...
                      2,
                      groups=in_channels,
                      bias=False), norm_layer(in_channels),
...
class E_FFN(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc1 = ConvBNReLU(in_channels=in_features,
                              out_channels=hidden_features,
                              kernel_size=1)
        self.conv1 = ConvBNReLU(in_channels=hidden_features,
                                out_channels=hidden_features,
                                kernel_size=ksize,
                                groups=hidden_features)
        self.conv2 = ConvBNReLU(in_channels=hidden_features,
                                out_channels=hidden_features,
                                kernel_size=3,
                                groups=hidden_features)
        self.fc2 = ConvBN(in_channels=hidden_features,
...
class MutilScal(nn.Module):
    ...
    def __init__(self, dim=512, fc_ratio=4, dilation=[3, 5, 7], pool_ratio=16):
        ...
        self.conv0_2 = nn.Conv2d(dim // fc_ratio,
                                 dim // fc_ratio,
                                 3,
                                 padding=dilation[-3],
                                 dilation=dilation[-3],
        ...
        self.conv1_2 = nn.Conv2d(dim // fc_ratio,
                                 dim // fc_ratio,
                                 3,
                                 padding=dilation[-2],
                                 dilation=dilation[-2],
        ...
        self.conv2_2 = nn.Conv2d(dim // fc_ratio,
                                 dim // fc_ratio,
                                 3,
                                 padding=dilation[-1],
                                 dilation=dilation[-1],
...
class Mutilscal_MHSA(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc = nn.Sequential(
            nn.Conv2d(in_channels=dim,
                      out_channels=dim // fc_ratio,
                      kernel_size=1), nn.ReLU6(),
            nn.Conv2d(in_channels=dim // fc_ratio,
                      out_channels=dim,
    ...
    def forward(self, x):
        ...
        q = rearrange(x,
                      'b (h d) (hh) (ww) -> (b) h (hh ww) d',
                      h=self.num_heads,
                      d=C // self.num_heads,
                      hh=H,
                      ww=W)
        k, v = rearrange(kv,
                         'b (kv h d) (hh) (ww) -> kv (b) h (hh ww) d',
                         h=self.num_heads,
                         d=C // self.num_heads,
                         hh=H1,
                         ww=W1,
...
class Block(nn.Module):
    ...
    def __init__(self,
        ...
        self.mlp = E_FFN(in_features=dim,
...
class MAF(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1_1 = nn.Conv2d(dim // fc_ratio,
                                 dim // fc_ratio,
                                 3,
                                 padding=dilation[-3],
                                 dilation=dilation[-3],
        ...
        self.conv2_1 = nn.Conv2d(dim // fc_ratio,
                                 dim // fc_ratio,
                                 3,
                                 padding=dilation[-2],
                                 dilation=dilation[-2],
        ...
        self.conv3_1 = nn.Conv2d(dim // fc_ratio,
                                 dim // fc_ratio,
                                 3,
                                 padding=dilation[-1],
                                 dilation=dilation[-1],
        ...
        self.fc = nn.Sequential(nn.Conv2d(dim, dim // fc_ratio, 1, 1),
                                nn.ReLU6(),
                                nn.Conv2d(dim // fc_ratio, dim, 1, 1),
                                nn.Sigmoid())

        self.s_conv = nn.Conv2d(in_channels=2,
                                out_channels=1,
                                kernel_size=5,
                                padding=2)
...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.b4 = Block(dim=decode_channels,
                        num_heads=16,
                        mlp_ratio=4,
                        pool_ratio=16,
                        dilation=dilation[0])
        ...
        self.b3 = Block(dim=decode_channels,
                        num_heads=16,
                        mlp_ratio=4,
                        pool_ratio=16,
                        dilation=dilation[1])
        ...
        self.b2 = Block(dim=decode_channels,
                        num_heads=16,
                        mlp_ratio=4,
                        pool_ratio=16,
                        dilation=dilation[2])
        ...
        self.seg_head = MAF(encode_channels[-4],
                            fc_ratio=fc_ratio,
                            dilation=dilation[3],
                            dropout=dropout,
...
class CMTFNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.decoder = Decoder(encode_channels,
                               decode_channels,
                               dropout=dropout,
                               num_classes=num_classes)
