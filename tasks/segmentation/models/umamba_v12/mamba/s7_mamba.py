...
class Mamba(nn.Module):
    ...
    def __init__(
        ...
        if init_layer_scale is not None:
            self.gamma = nn.Parameter(init_layer_scale * torch.ones((d_model)),
                                      requires_grad=True)
        ...
        self.x_proj_b = nn.Linear(self.d_inner,
                                  self.dt_rank + self.d_state * 2,
                                  bias=False,
                                  **factory_kwargs)
        self.dt_proj_b = nn.Linear(self.dt_rank,
                                   self.d_inner,
                                   bias=True,
                                   **factory_kwargs)
        ...
        self.x_proj_r = nn.Linear(self.d_inner,
                                  self.dt_rank + self.d_state * 2,
                                  bias=False,
                                  **factory_kwargs)
        self.dt_proj_r = nn.Linear(self.dt_rank,
                                   self.d_inner,
                                   bias=True,
                                   **factory_kwargs)

        self.D_r = nn.Parameter(torch.ones(self.d_inner,
                                           device=device))  # Keep in fp32
        ...
        if self.use_fast_path and causal_conv1d_fn is not None and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v1":
                ...
                if not self.if_devide_out:
                    ...
                    out = F.linear(
                        rearrange(out + out_b.flip([-1]), "b d l -> b l d") /
                        2, self.out_proj.weight, self.out_proj.bias)
            ...
            B = rearrange(B, "(b l) dstate -> b dstate l",
                          l=seqlen).contiguous()
            C = rearrange(C, "(b l) dstate -> b dstate l",
                          l=seqlen).contiguous()
            assert self.activation in ["silu", "swish"]
            y = selective_scan_fn(
                x,
    ...
    def step(self, hidden_states, conv_state, ssm_state):
        ...
        if selective_state_update is None:
            ...
            y = selective_state_update(ssm_state,
                                       x,
                                       dt,
                                       A,
                                       B,
    ...
    def allocate_inference_cache(self,
        ...
        conv_state = torch.zeros(batch_size,
        ...
        ssm_state = torch.zeros(batch_size,
                                self.d_model * self.expand,
                                self.d_state,
                                device=device,
                                dtype=ssm_dtype)
