...
from models.SpectralGPT import create_model
...
def get_cfg(dataset_name=None):
    ...
    backbone_weights = f"{MS_ROOT_DIR}/Checkpoints/SpectralGPT/SpectralGPT.pth"
    # backbone_weights = f"{MS_ROOT_DIR}/Checkpoints/SpectralGPT/SpectralGPT+.pth"
    model = create_model(nb_classes=len(labels),
                         weight_path=backbone_weights,
                         pretrain=True)
