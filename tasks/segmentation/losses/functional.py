...
def focal_loss_with_logits(
    ...
    ce_loss = F.binary_cross_entropy_with_logits(output,
                                                 target,
                                                 reduction="none")
...
def soft_dice_score(output: torch.Tensor,
    ...
    if dims is not None:
        ...
        intersection = torch.sum(output * target)
        cardinality = torch.sum(output + target)
    dice_score = (2.0 * intersection + smooth) / (cardinality +
                                                  smooth).clamp_min(eps)
    return dice_score


def wing_loss(output: torch.Tensor,
              target: torch.Tensor,
              width=5,
              curvature=0.5,
              reduction="mean"):
    ...
    loss[idx_smaller] = width * torch.log(1 +
                                          diff_abs[idx_smaller] / curvature)
