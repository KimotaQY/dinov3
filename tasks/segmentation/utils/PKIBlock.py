...
class ConvModule(nn.Module):
    ...
    # 添加更多归一化类型时可扩展
        raise NotImplementedError(
            f"Normalization layer '{norm_cfg['type']}' is not implemented.")
    ...
    # 添加更多激活类型时可扩展
        raise NotImplementedError(
            f"Activation layer '{act_cfg['type']}' is not implemented.")
...
class Poly_Kernel_Inception_Block(nn.Module):
    ...
    def __init__(self,
        ...
        # 预处理卷积层
        self.pre_conv = ConvModule(in_channels,
                                   hidden_channels,
                                   1,
                                   1,
                                   0,
        ...
        # 深度可分离卷积层1
        self.dw_conv = ConvModule(hidden_channels,
                                  hidden_channels,
                                  kernel_sizes[0],
                                  1,
                                  ...
                                  act_cfg=None)
        # 深度可分离卷积层2
        self.dw_conv1 = ConvModule(hidden_channels,
                                   hidden_channels,
                                   kernel_sizes[1],
                                   1,
                                   autopad(kernel_sizes[1], None,
                                           dilations[1]),
                                   ...
                                   act_cfg=None)
        # 深度可分离卷积层3
        self.dw_conv2 = ConvModule(hidden_channels,
                                   hidden_channels,
                                   kernel_sizes[2],
                                   1,
                                   autopad(kernel_sizes[2], None,
                                           dilations[2]),
                                   ...
                                   act_cfg=None)
        # 深度可分离卷积层4
        self.dw_conv3 = ConvModule(hidden_channels,
                                   hidden_channels,
                                   kernel_sizes[3],
                                   1,
                                   autopad(kernel_sizes[3], None,
                                           dilations[3]),
                                   ...
                                   act_cfg=None)
        # 深度可分离卷积层5
        self.dw_conv4 = ConvModule(hidden_channels,
                                   hidden_channels,
                                   kernel_sizes[4],
                                   1,
                                   autopad(kernel_sizes[4], None,
                                           dilations[4]),
        ...
        # 点卷积层
        self.pw_conv = ConvModule(hidden_channels,
                                  hidden_channels,
                                  1,
                                  1,
                                  0,
                                  ...
                                  act_cfg=act_cfg)
        # 后处理卷积层
        self.post_conv = ConvModule(hidden_channels,
                                    out_channels,
                                    1,
                                    1,
                                    0,
    ...
    def forward(self, x):
        ...
        # 通过深度可分离卷积层并进行累加
        x = x + self.dw_conv1(x) + self.dw_conv2(x) + self.dw_conv3(
            x) + self.dw_conv4(x)
...
if __name__ == "__main__":
    ...
    model = Poly_Kernel_Inception_Block(in_channels=64,
                                        out_channels=64)  # 实例化模型
