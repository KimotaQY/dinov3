...
class InverseConv2DLayer(nn.Module):
    ...
    def __init__(self,
        ...
        # 可学习的卷积核 (PSF)
        self.psf_weight = nn.Parameter(
            torch.randn(1, self.in_channels, self.kernel_size,
                        self.kernel_size))
        # 可学习的偏置
        ...
        self.psf_weight.data = nn.functional.softmax(
            self.psf_weight.data.view(1, self.in_channels, -1),
            dim=-1).view(1, self.in_channels, self.kernel_size,
                         self.kernel_size)
    ...
    def forward(self, x):
        ...
        otf = self.psf_to_otf(self.psf_weight,
    ...
    def psf_to_otf(self, psf, shape):
        ...
        otf = torch.roll(otf,
                         (-int(psf.shape[-2] / 2), -int(psf.shape[-1] / 2)),
                         dims=(-2, -1))
    ...
    def zero_insert_upsample(self, x, scale=3):
        ...
        """
        z = torch.zeros((x.shape[0], x.shape[1], x.shape[2] * scale,
                         x.shape[3] * scale)).type_as(x)
...
if __name__ == "__main__":
    ...
    model = InverseConv2DLayer(in_channels=32,
                               out_channels=32,
                               kernel_size=5,
                               scale=2)
    output = model(x)
