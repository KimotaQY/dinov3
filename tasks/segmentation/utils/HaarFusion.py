...
class DWT_Function(Function):
    ...
    def forward(ctx, x, w_ll, w_lh, w_hl, w_hh):
        ...
        x_ll = torch.nn.functional.conv2d(x,
                                          w_ll.expand(dim, -1, -1, -1),
                                          stride=2,
                                          groups=dim)
        x_lh = torch.nn.functional.conv2d(x,
                                          w_lh.expand(dim, -1, -1, -1),
                                          stride=2,
                                          groups=dim)
        x_hl = torch.nn.functional.conv2d(x,
                                          w_hl.expand(dim, -1, -1, -1),
                                          stride=2,
                                          groups=dim)
        x_hh = torch.nn.functional.conv2d(x,
                                          w_hh.expand(dim, -1, -1, -1),
                                          stride=2,
                                          groups=dim)
        # 将四个分量按通道维度拼接起来
    ...
    def backward(ctx, dx):
        ...
        if ctx.needs_input_grad[0]:
            ...
            filters = torch.cat([w_ll, w_lh, w_hl, w_hh],
                                dim=0).repeat(C, 1, 1, 1)
            # 使用转置卷积进行上采样
            dx = torch.nn.functional.conv_transpose2d(dx,
                                                      filters,
                                                      stride=2,
                                                      groups=C)
...
class DWT_2D(nn.Module):
    ...
    def forward(self, x):
        ...
        return DWT_Function.apply(x, self.w_ll, self.w_lh, self.w_hl,
                                  self.w_hh)
...
class IDWT_Function(Function):
    ...
    def backward(ctx, dx):
        ...
        if ctx.needs_input_grad[0]:
            ...
            x_ll = torch.nn.functional.conv2d(dx,
                                              w_ll.unsqueeze(1).expand(
                                                  C, -1, -1, -1),
                                              stride=2,
                                              groups=C)
            x_lh = torch.nn.functional.conv2d(dx,
                                              w_lh.unsqueeze(1).expand(
                                                  C, -1, -1, -1),
                                              stride=2,
                                              groups=C)
            x_hl = torch.nn.functional.conv2d(dx,
                                              w_hl.unsqueeze(1).expand(
                                                  C, -1, -1, -1),
                                              stride=2,
                                              groups=C)
            x_hh = torch.nn.functional.conv2d(dx,
                                              w_hh.unsqueeze(1).expand(
                                                  C, -1, -1, -1),
                                              stride=2,
                                              groups=C)
            # 将四个分量按通道维度拼接起来
...
class Fusion(nn.Module):
    ...
    def __init__(self, in_channels, wave):
        ...
        self.convh1 = nn.Conv2d(in_channels * 3,
                                in_channels,
                                kernel_size=1,
                                stride=1,
                                padding=0,
        ...
        self.convh2 = nn.Conv2d(in_channels,
                                in_channels * 3,
                                kernel_size=1,
                                stride=1,
                                padding=0,
        ...
        self.convl = nn.Conv2d(in_channels * 2,
                               in_channels,
                               kernel_size=1,
                               stride=1,
                               padding=0,
    ...
    def forward(self, x1, x2):
        ...
        x_dwt = self.dwt(
            x1
        )  # torch.Size([1, 32, 32, 32])  ===> torch.Size([1, 128, 16, 16])
        ...
        out_idwt = self.idwt(
            out
        )  # torch.Size([1, 128, 16, 16]) ===> torch.Size([1, 32, 32, 32])
